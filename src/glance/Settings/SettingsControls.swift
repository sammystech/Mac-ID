...
/// Sensitivity of the printed-photo deny cue. Separate from `LivenessModePicker` because it is the
/// one liveness control whose calibration the user genuinely has to verify for themselves — see
/// `PrintedPhotoSensitivity`.
struct PrintedPhotoSensitivityPicker: View {
    @Binding var selection: PrintedPhotoSensitivity
    var isEnabled: Bool = true

    var body: some View {
        SettingsLabeledOptionRow(
            title: "Printed photos",
            subtitle: "Looks for the pattern a printed photo makes when the camera samples it. Test yours in Face Lab before relying on it."
        ) {
            ForEach(PrintedPhotoSensitivity.allCases) { level in
                SettingsOptionTile(
                    title: level.title,
                    ...
                    Image(systemName: iconName(for: level))
                        ...
                        .foregroundStyle(SettingsOptionTile<EmptyView>.previewTint(isSelected: selection == level))
                }
            }
        }
        .disabled(!isEnabled)
    ...
    private func iconName(for level: PrintedPhotoSensitivity) -> String {
        switch level {
        case .off: return "xmark.circle"
        case .standard: return "photo"
        case .strict: return "photo.stack"
        }
    }
}
