...
struct MacIDApp: App {
    ...
    private var settingsWindow: some Scene {
        ...
        return Window("Mac ID Settings", id: "settings") {
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // The licence gate comes before everything else. Face unlock already refuses to run unlicensed
        ...
        if LicenseManager.shared.isEntitled {
            ...
            LicenseGateWindow.present { [weak self] in
                self?.beginLicensedLaunch()
            }
        }
    }
    ...
    private func presentOnboardingGate() {
        ...
        OnboardingController.startFlow(
            resumingAt: AppSettings.shared.onboardingResumeStep,
    ...
    /// Asks for Touch ID once at launch, if face unlock needs it.
    ///
    /// The session key that decrypts the stored password is held only in memory, so every launch —
    /// every reboot, every quit — starts locked. Nothing used to re-acquire it automatically, and
    /// `FaceUnlockCoordinator` silently refuses to arm while the session is locked. The result was
    /// that face unlock appeared to just stop working after a restart, with the only cure buried in
    /// Settings, and the first lock screen after a reboot is exactly where you most want it.
    ///
    /// Deliberately only when it will actually be used: face unlock on, a password stored, and the
    /// session not already open. Otherwise launching the app would prompt for a fingerprint it has
    /// no use for.
    private func unlockSessionForFaceUnlockIfNeeded() {
        ...
        Task { [environment] in
            // A short beat so the prompt doesn't race the login-time window dance.
            try? await Task.sleep(for: .milliseconds(600))
            await environment.pocController.unlockSession()
        }
    }
    ...
    private var isBlockedByPostUpdateNotice: Bool {
        AppSettings.shared.hasCompletedOnboarding && !AppSettings.shared.hasAcknowledgedSecurityNotice
    }
