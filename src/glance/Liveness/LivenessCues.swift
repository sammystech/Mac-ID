...
enum LivenessCue: String, CaseIterable, Hashable, Identifiable {
    ...
    nonisolated var title: String {
        ...
        case .printedPhoto: return "Printed photo"
    ...
    nonisolated var role: LivenessCueRole {
        switch self {
        case .glossGlare, .printedPhoto, .deviceDetected: return .deny
    ...
    nonisolated var explanation: String {
        ...
        case .printedPhoto: return "A repeating pattern in the skin texture, on a matte, gamut-compressed surface — ink on paper, not a face."
...
/// How hard the printed-photo cue tries.
///
/// Exposed as a real setting rather than a compile-time constant because its calibration is the
/// least certain thing in the liveness set: it was tuned against simulated prints and photographic
/// negatives, never against this machine's camera or a sheet from this user's printer. The right
/// answer is knowable, but only by the person who can hold a printed photo up to the lens.
enum PrintedPhotoSensitivity: String, CaseIterable, Identifiable, Sendable {
    /// The cue is switched off entirely, not merely set unreachably high.
    case off
    /// Calibrated point: no false fires on any photographic face tested, catches a printed photo
    /// at typical screen frequencies.
    case standard
    /// Catches finer screens, at a higher risk of rejecting a real face — worth it only if you have
    /// checked in Face Lab that your own face stays well clear of it.
    case strict
    ...
    var summary: String {
        switch self {
        case .off: return "No printed-photo check."
        case .standard: return "Rejects a printed photo held up to the camera."
        case .strict: return "Also catches finer prints. Slightly likelier to reject you."
        }
    }
    ...
    /// Tuning with this sensitivity applied; everything else stays at the calibrated defaults.
    func applied(to base: LivenessTuning = .default) -> LivenessTuning {
        var tuning = base
        tuning.printLevel = level
        tuning.printFrames = frames
        return tuning
    }
}
...
struct LivenessTuning: Equatable {
    ...
    /// Set high, and paired with a long frame count, because this cue convicts on texture
    /// statistics rather than on seeing an object: a false fire locks the user out of their own
    /// Mac. Tune it against your own printed photo in Face Lab before trusting it in Heavy mode.
    var printLevel: Float = 0.5
    var printFrames: Int = 5
    ...
    nonisolated func level(for cue: LivenessCue) -> Float {
        ...
        case .printedPhoto: return printLevel
    ...
    nonisolated func frames(for cue: LivenessCue) -> Int {
        ...
        case .printedPhoto: return printFrames
...
enum LivenessDecision: Equatable {
    ...
    var denialReason: String? {
        ...
        case .printedPhoto: return "This looks like a printed photo rather than a real face."
...
nonisolated enum LivenessCues {
    nonisolated static func readings(
        ...
        [
            ...
            .printedPhoto: printedPhoto(window.last),
    ...
    /// Convicts only where a *patterned* surface coincides with a *matte, gamut-compressed* one.
    ///
    /// Either half alone has a mundane explanation. Periodic texture on its own shows up in
    /// patterned clothing and in compression artifacts; matte flatness on its own is just some
    /// people's skin under diffuse light. Requiring both — and then four frames of both — is what
    /// keeps this from rejecting a real face, which is the failure that actually costs the user
    /// something. A glossy print, which has no matte signal, is caught by `glossGlare` instead,
    /// and a photo still in a phone or a paper sheet with a visible border by `deviceDetected`.
    nonisolated static func printedPhoto(_ frame: LivenessFrame?) -> CueReading {
        guard let sample = frame?.print else { return .none }

        // Floor sits above the highest periodicity measured on real face photographs (0.23) and the
        // ceiling at the middle of the range measured on halftone-screened versions of those same
        // faces (0.31-0.63). Camera frames reach this code as uncompressed BGRA, so unlike those
        // test images they carry no JPEG block structure to inflate the reading — the calibration
        // is therefore conservative rather than tight.
        let texture = ramp(sample.texturePeriodicity, floor: 0.25, ceiling: 0.55)
        // Skin lit by a screen almost always returns *some* specular pixels; matte ink returns none.
        // Only a modifier, never a trigger: chroma spread was measured to overlap completely between
        // real and printed faces, so it is not used at all.
        let specularDeficit = 1 - ramp(sample.specularFraction, floor: 0.002, ceiling: 0.02)

        let level = texture * (0.6 + 0.4 * specularDeficit)
        // The moiré beat is the whole signal, and it simply isn't present below roughly 120px
        // of face — there aren't enough samples for the pattern to alias into.
        let confidence = ramp(Float(sample.cropPixelWidth), floor: 120, ceiling: 260)
        return CueReading(level: level, confidence: confidence)
    }
